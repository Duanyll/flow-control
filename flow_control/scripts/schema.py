...
def generate_schemas() -> dict[str, dict]:
    ...
    ``preprocess`` / ``pack`` / ``serve`` configs. ``import_builtin_trainers`` registers the
    ...
    """
    from flow_control.data.pack import PackConfig
    ...
    schemas["pack"] = TypeAdapter(PackConfig).json_schema()
