...
class CivitaiDataset(Dataset):
    ...
    def __init__(self, path: str):
        ...
        logger.info(
            f"CivitaiDataset initialized with {len(self.image_files)} items from {path}"
        )
    ...
    def __getitem__(self, index) -> dict:
        ...
        if not os.path.exists(text_path):
            logger.warning(
                f"Image {image_file} has no corresponding text file. Using empty prompt."
            )
            prompt = ""
        else:
        ...
        return {"__key__": image_file, "clean_image": image, "prompt": prompt}
