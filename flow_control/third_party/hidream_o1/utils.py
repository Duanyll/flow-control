...
#   (models/utils.py, commit 2c2d29ff729e48f33e41f49edfdbd81d5ac103b4, MIT license)
# Provides get_rope_index_fix_point (mRoPE position ids for mixed text/vision/gen
# sequences), the official PREDEFINED_RESOLUTIONS list, and reference-image
# resize helpers used by HiDream-O1's pipeline.
# Local changes: stripped UTF-8 BOM; no logic changes.
import os
import math
import torch
...
def _xxyy_relative_to_absolute_bbox(
...
def parse_layout_bboxes(
    layout_bboxes: Any, width: int, height: int
) -> List[Dict[str, Any]]:
    """Convert xxyy relative layout boxes into the training-side bbox layout format."""
    raw_boxes = _unwrap_boxes(layout_bboxes)
    if not isinstance(raw_boxes, list):
        raise ValueError(
            "layout_bboxes must be a list, or a dict containing one of: layout_bboxes/bboxes/boxes"
        )
...
def add_outer_border_keep_size(
    pil: Image.Image, color: Iterable[int], width: int
) -> Image.Image:
...
def resize_pilimage(pil_image, image_size, patch_size=16, resampler=Image.BICUBIC):
    ...
    if s1 < s2:
        pil_image = pil_image.resize(
            [new_size[0], round(height / s1)], resample=resampler
        )
        top = (round(height / s1) - new_size[1]) // 2
        pil_image = pil_image.crop((0, top, new_size[0], top + new_size[1]))
    else:
        pil_image = pil_image.resize(
            [round(width / s2), new_size[1]], resample=resampler
        )
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            for _ in range(image_nums + video_nums):
                ...
                if skip_vision_start_token[image_index - 1]:
                    ...
                    llm_pos_ids_list.append(
                        torch.stack([t_index, h_index, w_index]) + fix_point + st_idx
                    )
                    fix_point = 0
                else:
                    llm_pos_ids_list.append(
