...
#   (models/qwen3_vl_transformers.py, commit 2c2d29ff729e48f33e41f49edfdbd81d5ac103b4, MIT license)
# HiDream-O1's Pixel-level Unified Transformer: a Qwen3-VL-8B backbone extended with
# BottleneckPatchEmbed / TimestepEmbedder / FinalLayer for pixel-space flow matching.
#
# Local changes (marked with "flow-control patch"):
# 1. Qwen3VLTextRotaryEmbedding: transformers>=5.x removed "default" from
#    ROPE_INIT_FUNCTIONS and its _init_weights expects a
#    `compute_default_rope_parameters` staticmethod on RotaryEmbedding modules.
#    Added the staticmethod (copied from transformers 5.9
#    models/qwen3_vl/modeling_qwen3_vl.py) and used it when rope_type == "default".
#
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3VLTextConfig, device=None):
        ...
        self.config = config
        # flow-control patch: transformers>=5.x drops "default" from ROPE_INIT_FUNCTIONS.
    ...
    # flow-control patch: copied from transformers 5.9
    # models/qwen3_vl/modeling_qwen3_vl.py::Qwen3VLTextRotaryEmbedding.
    # transformers>=5.x _init_weights calls this on *RotaryEmbedding modules that
    # carry `original_inv_freq` when rope_type == "default".
    @staticmethod
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    # flow-control patch: dropped @auto_docstring (undocumented custom args print
    # [ERROR] lines at import time under transformers 5.x).
    @check_model_inputs
    def forward(
        self,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # flow-control patch: transformers>=5.x renamed `input_embeds` ->
        # `inputs_embeds` and dropped `cache_position` from create_causal_mask.
        # A prepared 4D mask (the hybrid-attention path) is still returned as-is.
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def _forward_generation(
        ...
        **kwargs,
        ...
        precomputed_deepstack_image_embeds = kwargs.pop(
        ...
        if pixel_values is not None:
            if (
                precomputed_image_embeds is not None
                and precomputed_deepstack_image_embeds is not None
            ):
                image_embeds = precomputed_image_embeds.to(
                    inputs_embeds.device, inputs_embeds.dtype
                )
                deepstack_image_embeds = [
                    deepstack_image_embed.to(inputs_embeds.device, inputs_embeds.dtype)
                    for deepstack_image_embed in precomputed_deepstack_image_embeds
                ]
            else:
    ...
    # flow-control patch: dropped @auto_docstring — the diffusion-specific args
    # (vinputs/token_types/use_flash_attn/...) are undocumented and transformers 5.x
    # prints an [ERROR] line per arg at import time.
    @check_model_inputs
    def forward(
        self,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if vinputs is not None:
            return Qwen3VLCausalLMOutputWithPast(
                ...
                cond_deepstack_image_embeds=getattr(
                    outputs, "cond_deepstack_image_embeds", None
                ),
            )
