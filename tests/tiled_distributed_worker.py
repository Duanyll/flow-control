...
def main() -> None:
    ...
    try:
        ...
        wrapped = Tiled(tile_size=(4, 4), overlap=(2, 2)).wrap(adapter)
        x = torch.arange(height * width, dtype=torch.float32)[None, :, None]
        x.requires_grad_()
        velocity = wrapped.predict_velocity_batched(
            [{"image_size": (height, width), "noisy_latents": x}],
            [torch.tensor([0.5])],
        )[0]
        ...
        # Rank 0 has two real tiles and two discarded duplicates; rank 1 has four.
