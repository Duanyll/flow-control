...
| `latent_length` | Integer bucket/work estimate from `processor.get_latent_length`; may include text and reference tokens. It is not necessarily `noisy_latents.shape[1]`. | Offline `ProcessorStage.process`; adapter synthetic length-test samples. | `BucketDirectoryDatasink` bins on it at write time; the dataset recovers buckets from directory names. SFT's latent-length test. Resolution shift computes its own length and does not read this field. |
