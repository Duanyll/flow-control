...
3. Batched inference and evaluation is in `flow_control/training/inference.py`.
...
2. Behaviours are preferred to be methods of the config class.
3. To regenerate jsonc config schmema after editing the Pydantic models, run `uv run flow-control schema`.

### Logging and printing

1. Avoid using `print()` directly in library code. We use `rich` for fancy logging and printing. Import      `get_logger` for logging messages, and explicitly pass the `console` instance for any rich printing:
   ```python
   ...
   ```
   They have been configured to work correctly in multiprocessing and distributed training.
2. To print something in `if __name__ == "__main__":` block for quick testing and debugging, use `from rich import print` ONLY in that block.
...
2. Prefer to pass images as `torch.Tensor` in `BCHW` format and in `[0, 1]` range. You may find `pil_to_tensor` and `tensor_to_pil` functions in `flow_control.utils.tensor` useful.
3. True micro-batching is optionally implemented at `ModelAdapter` level, outer code could make use of it for throughput. Non-batched sequential operations are acceptable in non-performance-critical code.

## Output layout

Only `examples/` is tracked; `local/` (machine-specific launchers + in-progress
experiment configs), `data/`, `demo/`, `draft/` are gitignored. Read docs/repo-layout.md
for details.
...
4. Review your changes against the following criteria with subagents:
   - Is it correct?
   - Is it minimal? Is all repeated code removed?
   - Is it consistent with other parts of the codebase?
   - We need clear and useful warning/error messages, but do not guard against untriggerable corner cases.
   - Are the tests non-trival and useful? If not, remove it.