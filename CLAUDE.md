...
Top-level directories and what belongs in each. Only `examples/` is tracked; the
rest are gitignored (see `.gitignore`).

1. `examples/` — **tracked**, machine-neutral canonical reference configs (jsonc),
   ...
   machine after the data is staged. This is the only config tree that lands on
   `main`.
2. `local/` — **gitignored**, machine-specific launchers and in-progress
   experiment/tuning configs. Structure:
   ...
   the local copy.
3. `data/` — **gitignored**, datasets and preprocessed latent/text caches.
4. `demo/` — **gitignored**, generated outputs (`.pt`/`.png`) from running demo
   pipelines.
5. `draft/` — **gitignored**, one-off scratch scripts, benchmark dumps, notes, and
   throwaway checkpoints.

## Multi-machine workflow

This is a solo project worked on from several machines at once. We keep `main`
